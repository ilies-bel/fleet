...
Vite proxies `/_qa/` to `localhost:4000`, so you get live data from the running gateway with full HMR.
...
```bash
docker build -f gateway/Dockerfile -t qa-gateway .
```
...
Requires Docker daemon running (needs `/var/run/docker.sock`). The gateway will use whatever `qa-net` network and containers are already running.

---

## Adding API Endpoints

All feature management endpoints live in `gateway/src/api.js`. The file exports a single Express router mounted at `/_qa/api/` in `index.js`.
...
export async function myEndpoint(name) {
  const r = await fetch(`/_qa/api/features/${name}/my-endpoint`);
...
```bash
docker build -f Dockerfile.feature-base -t qa-feature-base .
```

All existing feature containers continue using the old image. New containers (via `qa-add.sh`) will use the updated image. Existing containers can be recreated with `qa-teardown.sh <name>` + `qa-add.sh <name> <branch>`.
...
```bash
docker cp config/entrypoint.sh qa-<name>:/entrypoint.sh
docker exec qa-<name> rm /tmp/.qa-built   # clear sentinel so it re-runs
docker restart qa-<name>
```

### Sentinel file

`/tmp/.qa-built` prevents the 7-stage build from running again on restart. To force a full rebuild of a running container:

```bash
docker exec qa-<name> rm /tmp/.qa-built
docker restart qa-<name>
```
...
|-------------|---------------|
| `__QA_BACKEND_URL__` | `/backend` |
| `__QA_APP_URL__` | `localhost:3000` |

Set these in your Next.js config as `process.env.NEXT_PUBLIC_BACKEND_URL ?? '__QA_BACKEND_URL__'` so they work both in QA fleet and in production.
...
Scripts source `.qa-config` to get `APP_ROOT`:
...
CONFIG_FILE="$SCRIPT_DIR/../.qa-config"

if [[ ! -f "$CONFIG_FILE" ]]; then
  echo "ERROR: .qa-config not found. Run qa-init.sh first." >&2
  exit 1
fi
...
```bash
docker build -f gateway/Dockerfile -t qa-gateway .
docker stop qa-gateway-container
docker rm qa-gateway-container
# Then restart via qa-init.sh or manually:
docker run -d \
...
```bash
curl -s http://localhost:4000/_qa/api/features | jq .
curl -s http://localhost:4000/_qa/api/status | jq .
```
...
```bash
curl -v http://localhost:3000/some/path 2>&1 | grep -E 'X-QA|HTTP|Location'
```

The proxy sets `X-QA-Feature` on the forwarded request.

### Check Docker socket connectivity

```bash
docker exec qa-gateway-container node -e "
...
| `scripts/qa-add.sh` | ~166 |
| `scripts/qa-init.sh` | ~147 |
| `scripts/qa-teardown.sh` | ~128 |
