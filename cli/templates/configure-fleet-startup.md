...
description: Auto-tune qa-fleet.conf BACKEND_BUILD_CMD and BACKEND_RUN_CMD for the detected stack, then restart the feature container on main and verify the health endpoint.
user-invocable: true
---

Auto-tune `qa-fleet.conf` for this project, restart the `qa-main` container, and verify `/actuator/health`.
...
2. `qa-fleet.conf` — record current values for `BACKEND_DIR`, `FRONTEND_DIR`, `BACKEND_BUILD_CMD`, `BACKEND_RUN_CMD`, `PROJECT_NAME`, `PROXY_PORT`.
...
**`BACKEND_BUILD_CMD`**
- Start from the current value in `qa-fleet.conf`.
...
Print a unified diff (before → after) of the relevant lines in `qa-fleet.conf`.

Use AskUserQuestion: "Apply the above changes to qa-fleet.conf? [y/N]"
...
1. Write the updated `qa-fleet.conf` in place. Only change the keys identified in Step 2; leave all other lines untouched. The edit must be idempotent (running the command twice produces the same file).

2. From the qa-fleet repo root, restart the main feature container:
...
3. Wait for the backend to start (max 4 minutes). Poll every 30 seconds:
   ...
   ```bash
   docker logs --tail 5 qa-main 2>&1 | grep 'backend entered RUNNING state'
   ```
...
4. Once RUNNING, run the health check:
   ...
   ```bash
   PROXY_PORT=$(grep '^PROXY_PORT' qa-fleet.conf | cut -d= -f2 | tr -d '"' || echo 3000)
...
- The exact diff that was applied to `qa-fleet.conf` (or "No changes applied" if declined).
- Container status: output of `docker ps --filter name=qa-main --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'`.
...
- Only edit `qa-fleet.conf` in the project root. Touch nothing else.
- Do NOT push to git. Do NOT modify `.claude/`, `.beads/`, or any source code file.
- If `pom.xml`, `package.json`, and `application*.yml` are all missing, print "Stack not auto-detectable — please set BACKEND_BUILD_CMD and BACKEND_RUN_CMD manually in qa-fleet.conf." and exit cleanly.
