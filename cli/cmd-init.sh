...
# Existing fleet.conf files written before these keys existed still work:
...
detect_orchestrator() {
  ...
  if [ -n "${compose_file}" ]; then
    ...
    if [[ "$ans" =~ ^[Yy]$ ]]; then
      ...
      if ! grep -q 'fleet-gateway' "${compose_file}" 2>/dev/null; then
        cat >> "${compose_file}" <<COMPOSE_SNIPPET

  # Fleet gateway — added by fleet init
  fleet-gateway:
    image: fleet-gateway
...
COMPOSE_SNIPPET
        ...
        warn "fleet-gateway already present in ${compose_file} — skipping"
      fi
    fi
  fi
}
...
# ─── Auto-discover .env files and write/update .fleet-shared ─────────────────
...
# files, and writes results into the idempotent marker block in .fleet-shared.
#
# Globals read:  APP_ROOT, FRONTEND_DIR, BACKEND_DIR (may be empty)
# No prompts — safe for non-interactive / CI runs.
discover_env_files() {
  local shared_file="${APP_ROOT}/.fleet-shared"
  ...
  if [ ! -f "${shared_file}" ]; then
    ...
    {
      echo "# .fleet-shared — list non-tracked files to mount read-only into fleet containers"
