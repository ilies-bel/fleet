  ...
  if [ ! -f "${gi}" ]; then
    {
      ...
      echo '#'
      echo '# Committed: fleet.toml, Dockerfile.feature-base (and config/, fragments/).'
...
# ─── Generate railpack plans for vite and next services ──────────────────────
for i in "${!SVC_NAMES[@]}"; do
  case "${SVC_STACKS[$i]}" in vite|next) ;; *) continue ;; esac
