...
# infer_java_version POM_XML — prints java version or "21".
infer_java_version() {
  local pom="$1"
  python3 - "${pom}" <<'PY' 2>/dev/null || echo "21"
import sys, re
try:
    txt = open(sys.argv[1]).read()
    for tag in ('java.version', 'maven.compiler.target', 'maven.compiler.source'):
...
# infer_go_version GO_MOD — prints go version (X.Y) or "1.22".
infer_go_version() {
  local gomod="$1"
  python3 -c "
import sys, re
try:
    txt = open(sys.argv[1]).read()
    m = re.search(r'^go\s+(\d+\.\d+)', txt, re.M)
    if m: print(m.group(1)); sys.exit(0)
except Exception: pass
print('1.22')
" "${gomod}" 2>/dev/null || echo "1.22"
}

# infer_node_version SERVICE_DIR — prints node major version or "20".
infer_node_version() {
  local dir="$1"
  python3 - "${dir}/package.json" "${dir}/.nvmrc" <<'PY' 2>/dev/null || echo "20"
import sys, json, re
try:
    pkg = json.load(open(sys.argv[1]))
    node_spec = pkg.get("engines", {}).get("node", "")
    m = re.search(r'(\d+)', node_spec)
    if m: print(m.group(1)); sys.exit(0)
except Exception:
    pass
try:
    nvmrc = open(sys.argv[2]).read().strip()
    m = re.search(r'(\d+)', nvmrc)
    if m: print(m.group(1)); sys.exit(0)
except Exception:
    pass
print("20")
...
# render_dockerfile STACK SERVICE_DIR DEST_FILE — renders a stack template with
# inferred versions and writes to DEST_FILE.
render_dockerfile() {
  local stack="$1" svc_dir="$2" dest="$3"

  local template="${FLEET_ROOT}/cli/stacks/Dockerfile.${stack}"
  # gradle reuses spring template when no gradle-specific one exists
  if [ "${stack}" = "gradle" ] && [ ! -f "${template}" ]; then
    template="${FLEET_ROOT}/cli/stacks/Dockerfile.spring"
  fi

  if [ ! -f "${template}" ]; then
    warn "No template found for stack '${stack}' at ${template} — skipping"
    return 1
  fi

  # Infer versions; defaults match SKILL.md contract
  local JAVA_VERSION="21" GO_VERSION="1.22" NODE_VERSION="20" POSTGRES_VERSION="16"
  case "${stack}" in
    spring|gradle)
      JAVA_VERSION=$(infer_java_version "${svc_dir}/pom.xml")
      NODE_VERSION=$(infer_node_version "${svc_dir}")
      ;;
    go)
      GO_VERSION=$(infer_go_version "${svc_dir}/go.mod")
      ;;
    next|vite|node)
      NODE_VERSION=$(infer_node_version "${svc_dir}")
      ;;
  ...
  export JAVA_VERSION GO_VERSION NODE_VERSION POSTGRES_VERSION
  # These may already be set from the outer scope; provide safe defaults
  BACKEND_PORT="${BACKEND_PORT:-8081}"
  FRONTEND_PORT="${FRONTEND_PORT:-3000}"
  PROXY_PORT="${PROXY_PORT:-3000}"
  export BACKEND_PORT FRONTEND_PORT PROXY_PORT

  apply_stack_template "${template}" "${dest}"
  info "Rendered .fleet/Dockerfile.feature-base.${stack} (java=${JAVA_VERSION} node=${NODE_VERSION} go=${GO_VERSION})"
}
...
write_fleet_toml() {
...

  {
    ...
    # Unique stacks (preserve insertion order via seen list)
    local seen_stacks=""
    for i in "${!SVC_STACKS[@]}"; do
      stack="${SVC_STACKS[$i]}"
      if ! echo "${seen_stacks}" | grep -qF "|${stack}|"; then
        seen_stacks="${seen_stacks}|${stack}|"
        echo "[[stacks]]"
        echo "type       = \"${stack}\""
        echo "dockerfile = \".fleet/Dockerfile.feature-base.${stack}\""
        echo ""
      fi
    done
...
info "Generating per-stack Dockerfiles..."

# Track rendered stacks (associative array requires bash 4+; use a plain string instead
# for portability with macOS bash 3)
rendered_stacks=""

idx=0
while [ "${idx}" -lt "${#SVC_NAMES[@]}" ]; do
  svc_stack="${SVC_STACKS[$idx]}"
  svc_name="${SVC_NAMES[$idx]}"
  svc_abs_dir="${PROJECT_ROOT}/${SVC_DIRS[$idx]}"
  df_dest="${FLEET_DIR}/Dockerfile.feature-base.${svc_stack}"

  if echo "${rendered_stacks}" | grep -qF "|${svc_stack}|"; then
    info "Stack '${svc_stack}' already rendered — skipping for ${svc_name}"
  else
    # Set per-service port env vars before rendering
    export BACKEND_PORT="${SVC_PORTS[$idx]}"
    export FRONTEND_PORT="${SVC_PORTS[$idx]}"

    render_dockerfile "${svc_stack}" "${svc_abs_dir}" "${df_dest}"
    check_hot_reload "${svc_abs_dir}" "${svc_stack}"
    rendered_stacks="${rendered_stacks}|${svc_stack}|"
  fi
...
# ─── Build base images ────────────────────────────────────────────────────────
info "Building per-stack base images..."

# Iterate the unique stacks we rendered
echo "${rendered_stacks}" | tr '|' '\n' | while read -r stack; do
  [ -z "${stack}" ] && continue
