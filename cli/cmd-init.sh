...
# in common.sh uses it to locate per-service railpack plan files.
...
# Ignores fleet's per-project generated / machine-specific files while keeping
# the hand-maintained, project-shared config under version control:
#   - fleet.toml               (project topology: services, ports, stacks)
#   - Dockerfile.feature-base  (base image, assembled from fleet fragments)
#   - config/, fragments/      (project-specific overrides, if present)
# Idempotent: creates the file if missing; if present, appends any required
# lines that are not already there (preserves user customisations).
write_fleet_gitignore() {
  ...
  if [ ! -f "${gi}" ]; then
    {
      echo '# Auto-generated by `fleet init` — fleet per-project ignores.'
      echo '#'
      echo '# Committed: fleet.toml and per-service railpack plan files.'
      echo '# Ignored below: regenerable (`fleet add`) or machine-specific files.'
      for line in "${required[@]}"; do echo "${line}"; done
    } > "${gi}"
    info "Written: ${gi}"
    return
  fi

  # Migrate pre-2.0.4 ignores: older fleet versions ignored fleet.toml and
  # Dockerfile.feature-base, which should now be committed. Strip those exact
  # lines so reconfiguring an existing project adopts the new policy.
  local removed=0
  local -a obsolete=('fleet.toml' 'Dockerfile.feature-base')
  for line in "${obsolete[@]}"; do
    if grep -Fxq -- "${line}" "${gi}"; then
      # Portable in-place delete of the exact line (BSD + GNU sed).
      local tmp="${gi}.tmp.$$"
      grep -Fvx -- "${line}" "${gi}" > "${tmp}" && mv "${tmp}" "${gi}"
      removed=$((removed + 1))
    fi
  done
  ...
  if [ "${removed}" -gt 0 ]; then
    info ".fleet/.gitignore migrated — un-ignored ${removed} now-committed file(s)"
  fi
  if [ "${added}" -gt 0 ]; then
    info ".fleet/.gitignore updated — appended ${added} line(s)"
  elif [ "${removed}" -eq 0 ]; then
    info ".fleet/.gitignore already has all required entries"
  fi
}
...
# ─── Build per-service feature base images via railpack plans ────────────────

# build_feature_image requires a railpack plan for each service; it errors
# clearly if the plan is absent (run fleet init --override to regenerate).
for i in "${!SVC_NAMES[@]}"; do
