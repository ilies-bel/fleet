...
Feature containers run on the internal Docker network (`fleet-net`) only — no host port exposure. You point your browser at port 3000 to interact with the app, and port 4000 to manage which feature is active.

```
localhost:4000            ← dashboard & API
localhost:3000            ← transparent proxy → active feature container
                                                  ↓
                                          fleet-<name>:3000 (internal)
```
...
3. Start a `fleet-<name>` container that builds internally from the worktree
...
## Configuration (`fleet.conf`)

Generated by the `fleet init` wizard, or copy `fleet.conf.example` manually. Key fields:
