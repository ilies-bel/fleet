...
router.post('/features/:key/start', async (req, res) => {
  ...
  appendEvent(opId, { message: 'start started' });
  try {
    await startContainer(`fleet-${key}`);
    updateStatus(key, 'starting');
    appendEvent(opId, { message: 'container starting' });
    endOperation(opId, { outcome: 'success' });
    res.json({ ok: true });
  } catch (err) {
    endOperation(opId, { outcome: 'failure', error: err });
...
router.get('/features/:key/diff', async (req, res) => {
  ...
  // Cluster-hosted features have no local worktreePath — skip docker exec entirely.
  // Locally-hosted features always have worktreePath set; the in-container exec
  // is the source of truth for whether git is available (the gateway process runs
  // inside Docker and cannot statSync host paths, so gitDir is not a reliable gate).
