    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      {clusters.map(cluster => (
        <div key={cluster.reasonCode} className="cluster-card" style={{
          ...
          <div style={{ color: 'var(--color-muted)', fontSize: '0.65rem', marginBottom: cluster.sampleKeys.length ? '0.4rem' : 0 }}>
            last seen {cluster.lastSeenAt ? new Date(cluster.lastSeenAt).toISOString() : '—'}
          </div>
          {cluster.sampleKeys.length > 0 && (
            <ul style={{ margin: 0, padding: '0 0 0 1rem', fontSize: '0.7rem', color: '#888' }}>
